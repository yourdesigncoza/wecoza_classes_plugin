...
(function($) {
    ...
    /**
     * Process legacy schedule data and convert to new format
     * Handles multiple legacy data formats
     */
    function processLegacyScheduleData(data) {
        // Detect data format
        const format = detectScheduleDataFormat(data);

        switch (format) {
            case 'legacy_v1':
                return convertLegacyV1Format(data);
            case 'legacy_form_values':
                return convertFormValuesFormat(data);
            case 'current_v2':
                return data; // Already in current format
            default:
                console.warn('Unknown schedule data format, treating as legacy');
                return convertUnknownLegacyFormat(data);
        }
    }

    /**
     * Detect the format of schedule data
     */
    function detectScheduleDataFormat(data) {
        if (!data || typeof data !== 'object') {
            return 'unknown';
        }

        // Check for current v2 format
        if (data.version === '2.0' && data.timeData) {
            return 'current_v2';
        }

        // Check for form values format
        if (data.format === 'legacy_form_values') {
            return 'legacy_form_values';
        }

        // Check for legacy v1 format (array of schedule entries)
        if (Array.isArray(data) || (data.schedule && Array.isArray(data.schedule))) {
            return 'legacy_v1';
        }

        // Check for simple object with start_time/end_time
     ...
     * Legacy v1: Array of schedule entries or simple object with times
     */
    function convertLegacyV1Format(data) {
        ...
        // Handle array format (multiple schedule entries)
        if (Array.isArray(data)) {
            // Extract common time from first entry
            if (data.length > 0 && data[0].start_time && data[0].end_time) {
                converted.timeData.startTime = data[0].start_time;
                converted.timeData.endTime = data[0].end_time;
                converted.timeData.duration = calculateTimeDuration(data[0].start_time, data[0].end_time).toFixed(1);
            }
        } else {
            // Handle simple object format
            if (data.start_time && data.end_time) {
                converted.timeData.startTime = data.start_time;
                converted.timeData.endTime = data.end_time;
                converted.timeData.duration = calculateTimeDuration(data.start_time, data.end_time).toFixed(1);
            }
        }
    ...
    /**
     * Convert form values format to current format
     */
    function convertFormValuesFormat(data) {
        ...
        return {
            ...
            timeData: {
                ...
                duration: data.start_time && data.end_time ?
                    calculateTimeDuration(data.start_time, data.end_time).toFixed(1) : ''
            },
            exceptionDates: [],
            holidayOverrides: {},
            lastUpdated: new Date().toISOString()
    ...
    /**
     * Convert unknown legacy format - best effort conversion
     */
    function convertUnknownLegacyFormat(data) {
        ...
        if (data.start_time) converted.timeData.startTime = data.start_time;
        if (data.end_time) converted.timeData.endTime = data.end_time;
        if (data.pattern) converted.pattern = data.pattern;
        if (data.start_date) converted.startDate = data.start_date;
        if (data.end_date) converted.endDate = data.end_date;

        // Calculate duration if we have times
        if (converted.timeData.startTime && converted.timeData.endTime) {
            converted.timeData.duration = calculateTimeDuration(
                converted.timeData.startTime,
                converted.timeData.endTime
            ).toFixed(1);
        }
     ...
     * Ensures older backend code can still process the data
     */
    function addLegacyCompatibilityFields($container, scheduleData) {
        // Legacy field names that older backend code might expect

        // Basic schedule fields (legacy format)
        createHiddenField($container, 'schedule_pattern', scheduleData.pattern);
        createHiddenField($container, 'schedule_start_date', scheduleData.startDate);
        createHiddenField($container, 'schedule_end_date', scheduleData.endDate);

        // Time fields - use single time format for legacy compatibility
        if (scheduleData.timeData.mode === 'single') {
            createHiddenField($container, 'schedule_start_time', scheduleData.timeData.startTime);
            createHiddenField($container, 'schedule_end_time', scheduleData.timeData.endTime);
            createHiddenField($container, 'schedule_duration', scheduleData.timeData.duration);
        } else {
            // For per-day mode, use the first day's times as legacy fallback
            const perDayTimes = scheduleData.timeData.perDayTimes;
            const firstDay = Object.keys(perDayTimes)[0];

            if (firstDay && perDayTimes[firstDay]) {
                createHiddenField($container, 'schedule_start_time', perDayTimes[firstDay].startTime);
                createHiddenField($container, 'schedule_end_time', perDayTimes[firstDay].endTime);
                createHiddenField($container, 'schedule_duration', perDayTimes[firstDay].duration.toFixed(1));
            }
        }

        // Selected days as comma-separated string (legacy format)
        if (scheduleData.selectedDays.length > 0) {
            createHiddenField($container, 'selected_days', scheduleData.selectedDays.join(','));
        }

        // Exception dates in legacy format
        if (scheduleData.exceptionDates.length > 0) {
            const exceptionDatesLegacy = scheduleData.exceptionDates.map(ex => ex.date).join(',');
            createHiddenField($container, 'exception_dates_legacy', exceptionDatesLegacy);
        }
    }

    /**
     * Ensure backward compatibility when form data is collected
     * This function can be called by external code that expects legacy format
     */
    function getLegacyCompatibleScheduleData() {
        const currentData = collectScheduleData();
        ...
        const legacyData = {
            ...
            selected_days: currentData.selectedDays,
            exception_dates: currentData.exceptionDates.map(ex => ex.date),
            holiday_overrides: currentData.holidayOverrides
        };

        // Add time data in legacy format
        if (currentData.timeData.mode === 'single') {
            legacyData.start_time = currentData.timeData.startTime;
            legacyData.end_time = currentData.timeData.endTime;
            legacyData.duration = currentData.timeData.duration;
        } else {
            // For per-day mode, create array of schedule entries (legacy format)
            legacyData.schedule_entries = [];

            Object.keys(currentData.timeData.perDayTimes).forEach(day => {
                const dayData = currentData.timeData.perDayTimes[day];
                legacyData.schedule_entries.push({
                    day: day,
            ...
            // Also provide first day's times as fallback
            const firstDay = Object.keys(currentData.timeData.perDayTimes)[0];
            if (firstDay) {
                const firstDayData = currentData.timeData.perDayTimes[firstDay];
                legacyData.start_time = firstDayData.startTime;
                legacyData.end_time = firstDayData.endTime;
                legacyData.duration = firstDayData.duration.toFixed(1);
            }
        }
    ...
    /**
     * Global function to get schedule data in legacy format
     * Can be called by other scripts that need backward compatibility
     */
    window.getScheduleDataLegacy = function() {
        return getLegacyCompatibleScheduleData();
    };
    ...
    function updateHiddenFormFields(scheduleData) {
        ...
        // Add legacy-compatible fields for backward compatibility
        addLegacyCompatibilityFields($container, scheduleData);
    }
